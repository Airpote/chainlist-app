...
{#if $currentChain}
	<div class="sticky top-0 flex w-full flex-wrap items-center gap-4 bg-base-300 p-4">
		<div class="icon">
		...
		<div class="flex flex-col gap-2">
			...
			{/if}
			<span class="ml-0.5 text-xl font-bold"
				>{$currentChain.name} - {$currentChain.nativeCurrency.symbol}</span
			>
			<div class="flex items-center gap-3">
				...
				{#if currentLlamaChain && currentLlamaChain.gecko_id}
					<a
						href="https://coinmarketcap.com/currencies/{currentLlamaChain.gecko_id}"
						target="_blank"
					...
					<a
						href="https://www.coingecko.com/en/coins/{currentLlamaChain.gecko_id}"
						target="_blank"
						class="external-link-secondary"
					>
						<CoinGeckoLogo />
		...
		<button
			class="btn btn-circle absolute right-4 top-4 sm:hidden"
			on:click={() => ($currentChain = null)}
		>
			<CloseIcon class="text-2xl" />
		</button>
		<button
			class="btn btn-primary ml-auto w-full gap-4 rounded-full px-8 text-lg"
			on:click={connect}
		>
...
<style lang="postcss">
	.external-link {
		@apply btn btn-xs rounded-full border text-sm;
